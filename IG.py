...
___logo___ = (f"""{H} 
 ...
 |  \| | ─▄─░█ ░█▀▀▀ ░█░█░█ ░█▀▀▀ ░█───  
 | |\  | ░█▄▄█ ░█▄▄▄ ░█▄▀▄█ ░█▄▄▄ ░█▄▄█   
 |_| \_|         PAID TOOL FREE USERS STAY OFF
       THIS TOOL IS REBUILD BY JEWEL CYBER TECH 
        THIS TOOL IS ONLY FOR EDUCATIONAL PROPOSE ANY ILLEGAL USE YOU FACE IT YOURSELF{P}
{B}[{P}>{B}]{P} {K}PAID TOOLS CRACK FROM IG PAY 15K TO ADIM MADE IN INDONENISA!{P}

{B}[{P}•{B}]{M}——————————————————————————————{P}
{B}[{P}>{B}]{P} {K}AU {P}: {K}{P}JEWEL TECH{M}
{B}[{P}>{B}]{P} {B}FB {P}: {B}JEWEL TECH.
{B}[{P}>{B}]{P} {U}IG {P}: {U}@jewelfund123
...
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        print(f"{B}[{P}!{B}]{P} {H}Wait for a minute...{P}");sleep(3);os.system('xdg-open https://wa.me/+2349055012862?text=Halo+Banh+Arya');exit()
    elif ___cookie in ['', ' ']:
